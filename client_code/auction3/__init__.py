from ._anvil_designer import auction3Template
from anvil import *
...
class auction3(auction3Template):
  def __init__(self, **properties):
    # Set Form properties and Data Bindings.
    self.init_components(**properties)
    self.auction_name = properties['name']
    self.auction_address = properties['address']
    self.auction_chain  = properties['chain']
  ...
  def refresh(self):
    ...
    if not self.auction_data["auctionEnded"]:
      self.button_place_bid.text = "Bid {:.3f} GOFURS".format(
        self.auction_data["nextMinimumBid"] / (10**18)
      )
    ...
    if event_args["sender"].text == "Finalize Auction":
      ...
      if self.contract_write is None:
        ...
        try:
          ...
          Swal.fire({"icon": "error", "title": "Yikes...", "text": m})
          event_args["sender"].enabled = True
